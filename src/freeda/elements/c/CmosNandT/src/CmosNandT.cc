// Thermal CMOS NAND Gate
// By Travis Lentz and Tony Mulder

#include "CmosNandT.h"

// Define the number of parameters
const unsigned CmosNandT :: n_par = 23;

// Element Information
ItemInfo CmosNandT::einfo =
{
  "cmosnandt",
  "CMOS NAND Gate, Electro-thermal",
...
// Constructor
CmosNandT::CmosNandT(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void CmosNandT::init() throw(string&)
{
...
void CmosNandT::getLocalRefIdx(UnsignedVector& local_ref_vec, TerminalVector& term_list)
{
...
// Evaluate Function
void CmosNandT::eval(AD * x, AD * effort, AD * flow)
{
  ...
  flow[1] = 0;
  flow[2] = 0;
  flow[3] = Idsn + Idsp1 + Idsp2;
  flow[4] = ((effort[0]-effort[3])*(Idsp1+Idsp2))-(effort[3]*Idsn)-(power*x[0])-lk;
}
