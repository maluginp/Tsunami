...
// Define the number of parameters : Static member
const unsigned CmosInv :: n_par = 71;

// Element Information
ItemInfo CmosInv::einfo =
{
  "cmosinv",
  "CMOS Inverter",
  "Shivam Priyadarshi",
...
// Constructor
CmosInv::CmosInv(const string& iname) : ADInterface(&einfo, pinfo, n_par, iname)
{
...
void CmosInv::init() throw(string&)
{
  // Set Constants
  scale = 1.0;
  ...
  if(vto_p == 0.0)
  {
    if(vfb_p != 2003.0)
      ...
      vto_p = -0.5*type_p; // Threshold of Pmos is Negative
  }
  else
    vto_p = type_p*vto_p;
  // //cout << "\n VTOP=" << vto_p << "\n";
  ...
  vtoT_n=vto_n-tcv_n*(tmp-tnom);
  vtoT_p=vto_p-tcv_p*(tmp-tnom);

  kpT_n=kp_n*pow(tmp/tnom,bex_n);
  kpT_p=kp_p*pow(tmp/tnom,bex_p);
  ucritT_n=ucrit_n*pow(tmp/tnom,ucex_n);
  ucritT_p=ucrit_p*pow(tmp/tnom,ucex_p);
  phiT_n=phi_n*tmp/tnom-3.0*vtT*log(tmp/tnom)-egTnom*tmp/tnom+egT;
  phiT_p=phi_p*tmp/tnom-3.0*vtT*log(tmp/tnom)-egTnom*tmp/tnom+egT;
  ibbT_n=ibb_n*(1+ibbt_n*(tmp-tnom));
  ibbT_p=ibb_p*(1+ibbt_p*(tmp-tnom));
...
// Evaluate Function
void CmosInv::eval(AD * x, AD * effort, AD * flow)
{
  ...
  //x[0]  : Vdd    x[3] : d(Vdd)/dt   Vdb of Nmos = x[2]  Vdb of Pmos = x[2]-x[0]
  //x[1]  : Vin    x[4] : d(Vin)/dt   Vgb of Nmos = x[1]  Vgb of Pmos = x[1]-x[0]
  //x[2]  : Vout   x[5] : d(Vout)/dt  Vsb of Nmos = 0     Vsb  of Pmos = 0
  //effort[0] : Vdd	flow[0] = AC+DC Current Entering in Vdd
  //effort[1] : Vin	flow[1] = AC+DC Current Entering in Gate
  //effort[2] : Vout      flow[2] = AC+DC Current Entering in Output
  ...
  double vtoa_n = vtoT_n + avto_n / sqrt(np_n * weff_n * ns_n * leff_n);
  double vtoa_p = vtoT_p + avto_p / sqrt(np_p * weff_p * ns_p * leff_p);
  double kpa_n = kpT_n * (1 + akp_n / sqrt(np_n * weff_n * ns_n * leff_n));
  double kpa_p = kpT_p * (1 + akp_p / sqrt(np_p * weff_p * ns_p * leff_p));
  ...
  double vc_n = ucritT_n * ns_n * leff_n;
  double vc_p = ucritT_p * ns_p * leff_p;
  ...
  double qbo_n = gammaa_n * sqrt(phiT_n);
  double qbo_p = gammaa_p * sqrt(phiT_p);
  double qox = 0.0;
  ...
  i_f_p = log(1 + exp((vp_p - type_p*0) / (2.0 * vtT)))*log(1 + exp((vp_p - type_p*0) / (2 * vtT))); // Vsb for pmos =0
  ...
  // Csd + Csg considered
  AD iqs_p = type_p*(-QS_p.fastAccessDx(0) * x[3] - QS_p.fastAccessDx(1) * x[4] - QS_p.fastAccessDx(2) * x[5]);
  ...
  AD iqg_p =  type_p*(-QG_p.fastAccessDx(0) * x[3] - QG_p.fastAccessDx(2) * x[5]);

  //AD iqg_n = type_n*(QG_n.fastAccessDx(1) * x[4] + QG_n.fastAccessDx(2) * x[5]);
  AD iqg_n = type_n*(-QG_n.fastAccessDx(2) * x[5]);
  ...
  AD iqd_p =  type_p*(-QD_p.fastAccessDx(0) * x[3] - QD_p.fastAccessDx(1) * x[4]);

  //AD iqd_n = type_n*(QD_n.fastAccessDx(1) * x[4] + QD_n.fastAccessDx(2) * x[5]); ;
  AD iqd_n = type_n*(-QD_n.fastAccessDx(1) * x[4]);

  // Final Current Assignments (DC+AC)
  flow[0] =   -(id_p + iqs_p); //  DC + AC  current entering in VDD
  flow[1] =   iqg_p  + iqg_n; // DC + AC gate current
  flow[2] = id_n + id_p + iqd_p + iqd_n ; // DC + AC output current
