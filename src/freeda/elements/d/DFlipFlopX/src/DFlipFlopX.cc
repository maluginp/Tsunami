...
#include "DFlipFlopX.h"
#include <cstdio>

// Set the number of parameters which is 4 for this element
const unsigned DFlipFlopX::n_par = 4;

// Element information
ItemInfo DFlipFlopX::einfo =
{
  "dflipflopx",
...
DFlipFlopX::DFlipFlopX(const string& iname) : Element(&einfo, pinfo, n_par, iname)
{
...
void DFlipFlopX::init() throw(string&)
{
   C = 1e-12;
   Rs = 10;
  ...
  // Add Source Resistance 1
     unsigned newelem_id =
      cir->addElement("resistor", getInstanceName() + ":Rs1:", true);
   ...
   // Add Source Resistance 2
      newelem_id =
      cir->addElement("resistor", getInstanceName() + ":Rs2:", true);
      // Connect to input terminal
   ...
   newelem_id =
   cir->addElement("capacitor", getInstanceName() + ":Cs1:", true);
   ...
   // Add Source Capacitance 2
      newelem_id =
      cir->addElement("capacitor", getInstanceName() + ":Cs2:", true);
     ...
     // Add Nand Gate 1 : At the Input
      ...
      cir->connect(newelem_id, term_id_i2);   // D  Connect
      cir->connect(newelem_id, term_id_i3);   // CLK Connect
       // Add an internal Terminal
   ...
   // Add  Capacitance at Internal Node 4
     ...
     // Add Nand Gate 3
      ...
      newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand3:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
  ...
  // Add Nand Gate 2
      ...
      // Add Nand Gate 4
      newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand4:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
...
// Add  Capacitance at Internal Node 7
   ...
   // Add Inverter
      ...
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
      cir->connect(newelem_id, term_id_i3);   // CLK
...
// Add Internal Node 9, 10,
  ...
  newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand5:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
 ...
 // Add  Nand Gate 6
  newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand6:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
      cir->connect(newelem_id, term_id_i8);   // In1
      cir->connect(newelem_id, term_id_i7);   // In2
...
// Add  Nand Gate 7
  newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand7:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
...
// Add  Nand Gate 8
  newelem_id =
      cir->addElement("cmos2nandx", getInstanceName() + ":Nand8:", true);
      // Connect to terminals
      cir->connect(newelem_id, term_id1);   //Vdd Connect
